...
export class JoinRequestService {
  ...
  async updateJoinRequestStatus(id: string, status: 'approved' | 'rejected'): Promise<IJoinRequest | null> {
    ...
    if (joinRequest && status === 'approved') {
      const randowPassword = generateRandomPassword()
      // const tempPassword = await bcrypt.hash(randowPassword, 10); // Replace 'temporary_password' with your logic
     ...
     // Create a new user account
      ...
      // Send welcome email
      try {
        await emailService.sendWelcomeEmail(joinRequest.email, joinRequest.name, randowPassword);
      } catch (error) {
